...
export const GARMENT_CATEGORY_IDS = [
  "tops",
  "pants",
  "skirts",
  "one_piece",
...
export type GarmentCategory = (typeof GARMENT_CATEGORY_IDS)[number];
...
export interface GarmentCategoryGroup {
  ...
  label: string;
  icon?: string;
  subcategories: readonly GarmentSubcategoryOption[];
}

export const GARMENT_CATEGORY_CATALOG: readonly GarmentCategoryGroup[] = [
  {
    id: "tops",
    label: "上衣",
    subcategories: [
...
export const CATEGORY_LABELS: Readonly<Record<GarmentCategory, string>> =
  Object.fromEntries(GARMENT_CATEGORY_CATALOG.map((group) => [group.id, group.label])) as Record<GarmentCategory, string>;
