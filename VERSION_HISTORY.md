...
## 2026-07-18 / v2.1.30-test / P4-A — 微信小程序新首页只读与位置宿主

- 小程序休眠首页替换为与 App P1.4.1 同构的服务器只读首页：业务日期问候、天气内唯一地点、今日/明日、推荐/衣橱、七日按需、远期出行、真实图片 join 与计划/已穿/blocked 快照。
- 引入 account/date generation 与会话内稳定 mutation 会话；不增加业务缓存、Outbox、隐藏队列或乐观更新。登录默认进首页，底栏收口为“首页 / 穿搭 / + / 种草 / 设置”。
- 地点 Sheet 只在用户主动点击后请求微信隐私授权与粗略位置；坐标仅发服务器 `resolve-device`，用户确认候选后再设临时/常驻，手工城市始终可用。
