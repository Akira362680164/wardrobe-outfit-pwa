...
- **目的**：按用户要求更新项目长期 agent 规则，明确 UI 设计必须遵守项目 UI 规范，并补齐小程序开发的目录确认、共享契约、线上数据源、隐私边界和验证要求。
- **版本变更**：无；当前应用版本仍为 `2.1.8-test`。
- **改动文件**：`AGENTS.md`、`VERSION_HISTORY.md`。
- **改动说明**：新增 `UI 设计与交互规则`，将 `docs/designs/wardrobe-ui-spec.md` 作为 UI 规范唯一事实源，要求基于真实业务流截图维护页面结构，禁止无任务理由重排信息架构或手工改 HTML 预览；扩充微信小程序开发工具规则，要求先确认真实目录/分支、复用线上数据源和共享契约、生产域名前置验证、隐私边界和微信开发者工具验证口径。
- **验证结果**：`git diff --check` 通过。
- **风险门禁**：low（仅文档治理；不改运行时代码、不改线上 API、不改 Android、不打 APK）；未触发 subagent：用户未通知。
- **未验证风险**：未运行小程序 typecheck、微信开发者工具编译或 UI 规范生成脚本；本轮只更新长期 agent 规则。
...
- **目的**：按用户补充要求，把 ChatGPT 页面分析收敛为可执行 MD 任务包，明确三个未修复一级页的微信胶囊避让规则、UI token 取值、加号弹层截图链路、subagent 文件边界和母 agent 收口验证方式。
- **版本变更**：无；当前应用版本仍为 `2.1.8-test`。
- **改动文件**：`docs/agent-task-bundles/2026-07-09-wechat-miniprogram-ui-gap-execution-plan.md`、`VERSION_HISTORY.md`。
- **改动说明**：新增小程序 UI gap 完整执行计划，明确衣橱首页只做回归，套装/种草/设置标题顶部与微信胶囊顶部按运行时胶囊信息对齐；补充颜色、圆角、遮罩、玻璃、Icon 标准；规定 subagent A/B/C 分别只改套装、种草、设置文件组，母 agent 负责共享文件、合并、测试、截图和提交；补齐加号弹层及三个新建入口跳转截图矩阵。
- **验证结果**：只读核对主工程规则、用户经验档案、`README.md`、`package.json`、`VERSION_HISTORY.md`、UI 标准、小程序 token、三个一级页、create-sheet 和桌面截图目录；本轮为文档任务包，未运行小程序 typecheck 或 WeChat 编译。
- **风险门禁**：low（仅新增执行方案文档和版本历史；不改运行时代码、不改线上 API、不改 Android、不打 APK）；未触发 subagent：用户未通知执行实现。
- **未验证风险**：执行方案尚未在小程序中实施；顶部胶囊对齐、加号弹层跳转和截图矩阵需后续实现阶段验证。
...
- **执行 Agent**：Codex（未触发 subagent：用户未通知）。
- **目的**：按用户补充信息，把后续微信小程序版本开发所需的微信开发者工具 CLI 入口和操作边界记录为项目长期 agent 规则。
- **版本变更**：无；当前版本仍为 `2.1.4-test`。
- **改动文件**：`AGENTS.md`（本地 agent 规则文件，受 `.gitignore` 排除）、`VERSION_HISTORY.md`。
- **改动说明**：新增 `wechatide -c <clientName> -t <toolName>` 用法、skill 目录和工具分组摘要；明确云开发写入、云函数部署和体验版上传必须先获用户授权；小程序版本继续遵守线上唯一数据源与隐私边界。
- **验证结果**：文档检查通过；未改业务运行时代码。
- **风险门禁**：low（文档治理，不改运行时、不改构建、不打包）；未触发 subagent：用户未通知。
- **未验证风险**：未启动微信开发者工具或执行小程序编译，仅记录用户提供的 CLI 信息。

## 2026-07-07 / v2.1.4-test / Codex — 取消横屏版长期要求

- **执行 Agent**：Codex（未触发 subagent：用户未通知）。
- **目的**：按用户要求把“后续该应用不需要做横屏版”记录为项目长期 agent 规则。
- **版本变更**：无；当前版本仍为 `2.1.4-test`。
- **改动文件**：`AGENTS.md`（本地 agent 规则文件，受 `.gitignore` 排除）、`VERSION_HISTORY.md`。
- **改动说明**：移动端体验、视觉/交互验证、Android 功能验证和审查提示词均改为手机竖屏窄屏验收；横屏不再作为设计或验收目标，除非用户明确重新要求。
- **验证结果**：文档检查通过；确认 `AGENTS.md` 中横屏仅保留为“不再要求/除非用户明确要求”的边界说明。
- **风险门禁**：low（文档治理，不改业务运行时代码）；未触发 subagent：用户未通知。
- **未验证风险**：无业务代码变更，未运行类型检查、构建或 Android 验证。
...
# v2.1.3-test - 2026-07-01 — 生产清理命令修复
