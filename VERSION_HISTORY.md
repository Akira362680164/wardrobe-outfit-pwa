...
## 2026-07-01 / v2.1.2-test / Codex — 线上刷新状态组件接线与重复 Toast 清理
...
- **图片边界**：本提交不改 `OnlineImageClient`/Repository 的资产映射，不引入缓存或半成品资产引用；按需原图与局部重试仍需作为独立高风险数据层任务完成，不能由本次 UI 接线冒充完成。
- **验证**：将在提交前执行全量逻辑测试、类型检查、生产构建；随后构建固定签名 APK 并执行 Android 模拟器回归。
- **风险门禁**：**high**（线上刷新状态、版本与 Android 交付）。
- **未触发 subagent**：用户未通知。
- **未验证风险**：图片原图按需加载和单图失败局部重试未实现；真实测试服务器业务流程不因本次纯 UI 复用改造重新部署。
