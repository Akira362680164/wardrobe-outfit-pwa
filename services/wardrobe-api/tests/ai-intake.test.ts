  ...
  it("forwards batch garment recognition to the server-side MiniMax service", async () => {
    ...
    const response = await app.inject({
      ...
      payload: {
        ...
        items: [
          { clientItemId: "image-1", imageDataUrl: "data:image/png;base64,AAAA", fallbackName: "shirt.jpg" },
          { clientItemId: "image-2", imageDataUrl: "data:image/png;base64,BBBB", fallbackName: "pants.jpg" },
        ],
      },
    });
    ...
    expect(response.json()).toMatchObject({
      items: [
        { clientItemId: "image-1", status: "succeeded", tag: { category: "tops", candidateNames: ["白色衬衫"] } },
        { clientItemId: "image-2", status: "succeeded", tag: { category: "tops", candidateNames: ["白色衬衫"] } },
      ],
    });
    expect(calls).toHaveLength(1);
    expect(calls[0]).toMatchObject({ items: [{ clientItemId: "image-1" }, { clientItemId: "image-2" }], miniMax: { apiKey: "test-key" } });
