...
import { hardFilterGarments, mapGarmentRole } from "./engine.js";
import { RuleDateContextResolver } from "./ports.js";
import { RecommendationWorkspaceAdapter } from "./workspace-adapter.js";

export type RecommendationAcceptStage = "afterPlan" | "afterBindings" | "afterAction" | "beforeCommit";
type Selection = { garment: RecommendationGarment; role: GarmentSlot };
type SelectionValidator = (userId: string, date: string, selectedIds: readonly string[]) => Promise<Selection[]>;
...
export class RecommendationAcceptService {
  ...
  async accept(userId: string, deviceId: string, date: string, command: AcceptRecommendationCommand): Promise<AcceptRecommendationResponse> {
    ...
    const selection = await this.validateSelection(userId, date, command.selectedGarmentIds);
    ...
    try {
      ...
      const selectedRows = await client.query(`
        select g.id,g.payload,b.asset_id from garments g
        left join lateral (select asset_id from asset_bindings where user_id=g.user_id and owner_entity_type='garment' and owner_entity_id=g.id order by case when field_name in ('primaryImage','image','cover') then 0 else 1 end,id limit 1) b on true
      ...
      if (selectedRows.rowCount !== command.selectedGarmentIds.length || selectedRows.rows.some((row) => {
        const expected = selection.find((entry) => entry.garment.id === row.id)?.garment; const payload = row.payload ?? {};
      ...
      const garmentSnapshots = command.selectedGarmentIds.map((garmentId) => {
        ...
        return { garmentId, ...(Number.isInteger(payload.legacyItemId) ? { legacyItemId: payload.legacyItemId } : {}), name: String(payload.name ?? "未命名衣物").slice(0, 120), role: roleById.get(garmentId)!, category: selection.find((entry) => entry.garment.id === garmentId)!.garment.category!, ...(row.asset_id ? { imageAssetId: row.asset_id } : {}) };
      });
  ...
  private currentSelectionValidator(overview: WeatherOverviewService): SelectionValidator {
    const adapter = new RecommendationWorkspaceAdapter(this.pool); const resolver = new RuleDateContextResolver();
