...
export class PostgresWeatherCacheRepository implements WeatherCacheRepositoryLike {
  ...
  async write(key: WeatherCacheKey, value: WeatherCacheStored): Promise<void> {
    ...
    `, [...values(key), JSON.stringify(value.payload), value.providerUpdatedAt, value.fetchedAt, value.expiresAt, value.staleUntil, JSON.stringify(value.sources), JSON.stringify(value.license), value.targetLocalDate]);
  }
