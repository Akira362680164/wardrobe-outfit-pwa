...
Page({
  data: { loading: false, error: "", query: "", scope: "all", category: "all", garments: [] as MiniGarment[], results: [] as MiniGarment[], locations: [] as MiniClosetLocation[], locationById: {} as Record<string, string>, categories: [] as Array<{ value: string; label: string }>, history: [] as string[] },
  onLoad(this: any) { const history = wx.getStorageSync(HISTORY_KEY); this.setData({ history: Array.isArray(history) ? history.slice(0, 10) : [] }); void this.load(); },
