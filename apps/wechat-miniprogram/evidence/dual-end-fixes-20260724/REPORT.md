...
| DUAL-P2-002 | PASS | hero 使用 `aspectFit`、52vh 且 600–1000rpx 边界，缩略图和标题层级按 UI spec 收敛；保留微信胶囊和原生标题 |
...
| 字体放大 | PASS | `wechat-devtools-360x640-font-144pct-wardrobe.png`；开发者工具 23/16，约 144%，强于 130% 门槛 |
...
截图为开发者工具整窗证据，右侧模拟器展示正式页面，左侧工程树和底部控制台用于证明运行来源。早期网络失败截图已移出证据目录。
