...
async function openHome(page) { await page.goto(appOrigin, { waitUntil: "networkidle" }); if (await page.getByLabel("邮箱或手机号").isVisible().catch(() => false)) { await page.getByLabel("邮箱或手机号").fill("fixture111@example.test"); await page.getByLabel("密码").fill("FixturePassword123!"); await page.getByLabel("我已阅读并同意").check(); await page.getByRole("button", { name: "登录", exact: true }).click(); } await page.getByTestId("home-weather-today").waitFor(); }
