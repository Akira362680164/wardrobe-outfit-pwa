...
assert.match(markup, /保存为套装/);
assert.doesNotMatch(markup, /保存为我的套装|保存到我的套装/);
assert.match(markup, /\{\{recommendationSourceSummary\}\}/, "recommendation cards must render the date-specific legal location source summary");
assert.doesNotMatch(markup, /recommendationMode === 'forecast' \? '结合天气'/, "forecast cards must not replace their legal location source with a generic weather label");
