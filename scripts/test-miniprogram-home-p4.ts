...
assert.match(markup, /<cover-view class="weather-copy-overlay weather-copy-overlay--today">/, "today weather copy must remain above decorative Canvas");
assert.match(markup, /<canvas[^>]+type="2d"[^>]*\/>\s*<cover-view class="weather-copy-overlay weather-copy-overlay--today">/, "today weather copy must live directly after Canvas inside the same card");
assert.match(markup, /<cover-view class="weather-temperature weather-temperature--small">/, "today and tomorrow temperatures must use the same cover-view text renderer");
assert.match(markup, /<cover-view class="weather-summary">/, "today and tomorrow summaries must use the same cover-view text renderer");
assert.match(markup, /<cover-view class="weather-meta">/, "today and tomorrow metadata must use the same cover-view text renderer");
