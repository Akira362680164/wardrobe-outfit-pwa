#!/usr/bin/env tsx
import { execFileSync } from "child_process";
import * as path from "path";

async function main() {
  const runId = process.env.TEST_RUN_ID || "";
  const dbUrl = process.env.DATABASE_URL || "";
  const storageRoot = process.env.TEST_STORAGE_ROOT || "";

  if (!runId || !runId.startsWith("run_")) {
    console.error("TEST_RUN_ID must start with run_");
    process.exit(1);
  }

  if (!/^run_[a-z0-9_]+$/.test(runId)) {
    throw new Error("TEST_RUN_ID contains unsafe characters");
  }
  console.log(`Dropping schema: ${runId}`);
  execFileSync(
    "psql",
    [dbUrl, "-c", `DROP SCHEMA IF EXISTS "${runId}" CASCADE;`],
    { stdio: "inherit" },
  );

  if (storageRoot && storageRoot.startsWith(process.cwd())) {
    const resolved = path.resolve(storageRoot);
    if (resolved === process.cwd())
      throw new Error("Refusing to trash project root");
    console.log(`Moving test storage to Trash: ${resolved}`);
    execFileSync("trash", [resolved], { stdio: "inherit" });
  }

  // Verify public schema has no new business records
  const recordCount = execFileSync(
    "psql",
    [
      dbUrl,
      "-tAc",
      "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='public' AND table_type='BASE TABLE';",
    ],
    { encoding: "utf-8" },
  ).trim();
  console.log(`Public schema tables: ${recordCount}`);

  console.log("Cleanup complete");
}

main().catch((e) => {
  console.error(e);
  process.exit(1);
});
