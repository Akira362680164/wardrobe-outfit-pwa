...
try{
  ...
  await page.emulateMedia({reducedMotion:"reduce"});await page.waitForFunction(()=>window.__wardoraWeatherCanvas?.status==="reduced_static");const runtimeReduced=await page.evaluate(()=>window.__wardoraWeatherCanvas);assert(runtimeReduced.clock===0&&runtimeReduced.fps===0,`runtime reduced-motion did not reset to calm frame ${JSON.stringify(runtimeReduced)}`);await page.emulateMedia({reducedMotion:"no-preference"});await page.waitForTimeout(220);const resumed=await page.evaluate(()=>window.__wardoraWeatherCanvas);assert(resumed.clock<.4,`resume caught up hidden time ${JSON.stringify(resumed)}`);
  ...
  await page.keyboard.press("Escape");await page.getByRole("dialog",{name:"使用当前位置说明"}).waitFor({state:"hidden"});assert(await page.getByTestId("home-city-sheet").isVisible(),"first Back must close purpose Sheet only");await page.getByTestId("home-use-current-location").click();
  ...
  for(const width of [360,390,430])for(const [scale,fontSize] of [[100,"16px"],[130,"20.8px"]]){await page.setViewportSize({width,height:844});await page.evaluate(value=>{document.documentElement.style.fontSize=value},fontSize);const geometry=await page.evaluate(()=>{const buttons=[...document.querySelectorAll('[data-testid="home-device-location-candidates"] button')].map(element=>{const rect=element.getBoundingClientRect();return{left:rect.left,right:rect.right,top:rect.top,bottom:rect.bottom,height:rect.height}});const panel=document.querySelector('[data-testid="home-city-sheet"]')?.getBoundingClientRect();return{overflow:document.documentElement.scrollWidth-document.documentElement.clientWidth,buttons,panel:panel?{left:panel.left,right:panel.right,bottom:panel.bottom}:null}});assert(geometry.overflow<=1,`${width}px ${scale}% location sheet overflow ${geometry.overflow}`);assert(geometry.buttons.every(rect=>rect.left>=0&&rect.right<=width&&rect.height>=44),`${width}px ${scale}% controls clipped ${JSON.stringify(geometry)}`);await page.screenshot({path:`${evidenceDir}/location-${width}-font${scale}.png`,fullPage:true})}
