...
export function HomeWeatherCanvas({ visual, forecast, stale }: { visual: QWeatherVisualDefinition; forecast: boolean; stale: boolean }) {
  ...
  useEffect(() => {
    ...
    const scene = createWeatherScene(visual.code, "today");
    ...
    const drawStill = () => { try { resize(); renderWeatherScene(context, scene, width, height, performance.now(), false, true); updateDiagnostics(reduced.matches ? "reduced_static" : "static_ready"); } catch { fail(); } };
    ...
    const onReduced = () => { scheduler.setReducedMotion(reduced.matches); drawStill(); };
