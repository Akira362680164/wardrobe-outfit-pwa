...
import { AlertTriangle, CheckCircle2, ChevronLeft, ChevronRight, Loader2, Lock, LogOut, User } from "lucide-react";
...
import type { WorkspaceEntityType } from "@/lib/account-workspace-db";
      ...
      <div className="grid gap-2">
        ...
        {confirmingLogout ? (
          <div className="grid gap-2 rounded-lg border border-clay/30 bg-clay/5 p-3">
            <p className="text-sm font-semibold text-clay">确认退出登录？</p>
            <p className="text-xs text-ink/60">退出后你仍可以使用本机已缓存的数据，下次登录可恢复当前账号工作区。</p>
            <div className="grid grid-cols-2 gap-2">
              <button
                type="button"
                onClick={() => setConfirmingLogout(false)}
                disabled={auth.isBusy}
                className="h-10 rounded-lg border border-ink/10 text-sm disabled:opacity-60"
              >
                取消
              </button>
              <button
