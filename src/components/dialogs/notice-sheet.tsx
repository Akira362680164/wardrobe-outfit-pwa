...
export function NoticeSheet({ open, title, description, actionLabel = "知道了", onClose }: { open: boolean; title: string; description?: ReactNode; actionLabel?: string; onClose: () => void }) {
  return (
    ...
    >
      ...
      {description ? <div className="mt-2 whitespace-pre-wrap text-sm text-ink/55">{description}</div> : null}
      <AsyncActionButton
        label={actionLabel}
        className="mt-4 w-full"
        data-parity-id="parity.app.app.src.components.dialogs.notice.sheet.202beda68f"
        onClick={onClose}
      />
    </MotionSheet>
  );
