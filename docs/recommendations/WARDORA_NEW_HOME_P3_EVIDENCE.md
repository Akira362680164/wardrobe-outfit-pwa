...
| --- | --- | ---: | --- |
| 304 | 静帧、雷光 0.03/0.32、冰雹 0.4/0.7/0.9 | 0.05229–0.06050 | 今日 Canvas 1，明日 0 |
| 403 | 暴雪 clock 0/2.5 | 0.03229–0.03295 | 今日 Canvas 1，明日 0 |
| 508 | 强沙尘暴 clock 0/4 | 0.06728–0.06975 | 今日 Canvas 1，明日 0 |
| 512 | 重度霾 clock 0/8 | 0.03858–0.03890 | 今日 Canvas 1，明日 0 |
...
- `npm run test:visual:home-feed-p3`：13 个固定 code/clock 使用原型原生尺寸对照，不再以 `fit:fill` 拉伸；全部低于硬阈值（主要场景 `MAE≤0.08`、998 `≤0.03`），今日/明日结构通过。
- `npm run test:browser:home-feed-p3`：真实 rAF `28 FPS`、DPR `2`、离屏停止、运行时 reduced-motion 归零、恢复不补帧、用途确认前坐标解析请求 0、确认后 1；地点 Sheet 与完整首页均覆盖 360/390/430px × 100%/130%，无根级溢出或关键内容裁切，嵌套 Sheet Escape/Back 通过。
...
- 模拟器多轮 Canvas 实测 `18–26 FPS`，最终证据轮为 23 FPS；浏览器同一调度器实测 28 FPS。reduced-motion 为 `fps=0/clock=0`，恢复不补帧。该模拟器性能值如实保留，不以浏览器数值冒充 Android。
...
- 冻结提交：`a5b6d08c10b37effb2465c7048dd2a6b37ee650c`。
- 最终结论：P0 `0` / P1 `0` / P2 `0` / P3 `0`，建议交付。
- 审查确认首页已删除“主计划已保护 / 天气变化只提示风险 / 不会自动更换”等无操作价值的内部说明；所有首页错误路径改为固定、可执行的用户文案，城市搜索 429 亦不再透传服务端自由 message。
- 回归门禁禁止上述内部文案、`onlineErrorMessage` 和 `message: error.message` 再进入首页控制器及地点操作层。
