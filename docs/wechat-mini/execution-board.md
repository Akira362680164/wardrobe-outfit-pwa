...
- 历史上已完成的微信认证登录后端基础保留，但后续小程序开发先消费现有接口，不继续改后端契约。
...
| 微信登录后端 | subagent-auth-backend | done | `packages/cloud-contracts/src/auth/**`, `services/wardrobe-api/src/auth/**`, migrations/tests | 微信认证登录契约、服务端 mock 测试和 SQL migration 完成 |
