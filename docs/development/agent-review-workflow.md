...
- 只有用户明确要求“启动 subagent”“独立审核”“并行 Agent”或同等意思时，才允许启动 subagent。用户已对本项目后续 UI/视觉/动效修复给予常驻明确授权，视觉修复执行 Session 按下述专项流程自行启动一个独立只读验证 subagent，不需要每次重复询问。
- 不得因为风险等级、改动规模或 Agent 自己的判断自动委派。
- 除上述视觉修复常驻授权外，用户没有明确通知时，所有风险等级都由当前 Agent 通过本地验证收口，并在版本历史中写明“未触发 subagent：用户未通知”。

### 1.1 UI/视觉/动效修复的独立验证流程

执行 Session 不得在自己的实现上下文内同时宣布视觉验收通过。正确顺序是：

1. 执行 Session 完成首轮修改、自测和截图/录屏证据；
2. 将待审代码提交为冻结 commit，未提交工作区不得作为独立审查基线；
3. 由执行 Session 自行启动一个独立只读视觉验证 subagent；
4. 审查 subagent 使用独立分支、worktree 和目录，只读取冻结 commit，不修代码；
5. 执行 Session 修复报告中本轮范围内的 P0/P1 和经确认的 P2，复跑验收后再合入正式基线。

为减少执行上下文对审查判断的锚定，交给审查 subagent 的信息限于：

- 权威 PRD、UI 规范、HTML 原型、参考截图及对应 SHA/版本；
- 待审冻结 commit、改动文件范围与禁止扩展项；
- 必须覆盖的 viewport、字体比例、页面状态、平台与证据目录；
- 已生成的截图、录屏、manifest 和可重现命令。

不得交给审查 subagent：执行 Session 的实现推理、对自己成果的评价、期望审查通过的引导性语句，或要求审查者顺手修代码的指令。

视觉审查至少输出：

1. 按 P0–P3 排序的问题，包含页面/组件、证据和用户影响；
2. 原型/规范与实现的关键差异；
3. 响应式、字体放大、滚动/溢出、触摸区、动效、reduced-motion、loading/empty/error/permission 的未验证项；
4. 明确的“建议交付 / 修复后再交付”结论。
